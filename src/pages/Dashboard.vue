<template>
    <Block v-if="this.state.data.spotifyToken == null && this.state.data.showSpotify" class="mb-4 bg-green-400 text-white font-bold">
...
export default {
    ...
    methods: {
        removeFromQueue(uniqueId) {
            this.state.removeFromQueue(uniqueId);
        }
    }
}
</script>
